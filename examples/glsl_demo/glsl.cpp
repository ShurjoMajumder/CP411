...
{
    if (!useGLSL) return const_cast<char *>(aGLSLStrings[0].c_str());
 
 GLint blen = 0;    // bugfix Oct-06-2006	
 GLsizei slen = 0;  // bugfix Oct-06-2006

 if (ProgramObject==0) return const_cast<char *>(aGLSLStrings[2].c_str());
    ...
    if ((linker_log = (GLcharARB*)malloc(blen)) == NULL) 
     {
        ...
        return const_cast<char *>(aGLSLStrings[3].c_str());
    }
 ...
 if (linker_log!=0)
    ...
    return const_cast<char *>(aGLSLStrings[5].c_str());
   
 return const_cast<char *>(aGLSLStrings[4].c_str());
}
...
if (!useGLSL) return const_cast<char *>(aGLSLStrings[0].c_str());
 ...
 if (ShaderObject==0) return const_cast<char *>(aGLSLStrings[1].c_str()); // not a valid program object
 ...
 if (compiler_log!=0)
   ...
   return const_cast<char *>(aGLSLStrings[6].c_str());
